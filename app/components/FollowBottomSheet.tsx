...
const FollowBottomSheet: React.FC<ModalArgs> = ({ dismiss, visible }) => {
  const [selectedFollow, setSelectedFollow] = useState<string | null>(null);

  const parameters = ["Unfollow", "Restrict", "Block"];
    ...
    >
      ...
      <View style={styles.sheet}>
        ...
        {parameters.map((param) => {
          const selected = selectedFollow === param;
          return (
            <TouchableOpacity
              key={param}
              onPress={() => setSelectedFollow(param)}
              style={styles.option}
            >
              <AppText variant="body1Light" style={styles.optionText}>{param}</AppText>
              <View
                style={[
                  styles.radioOuter,
